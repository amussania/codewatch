...
function CellContent({ val, isCW }: { val: CellVal; isCW?: boolean }) {
  ...
  if (isCW)             return <span className="font-sans text-[#00c4a0] font-semibold text-xs tracking-[.04em]">{val}</span>;
  return <span className="font-sans text-[#8896ab] text-xs">{val}</span>;
}
    ...
    <section className="py-[120px] bg-[#fafafa]">
      <div className="max-w-[1100px] mx-auto px-8">
        ...
        <motion.div
          ...
          transition={{ duration: 0.55, ease: "easeOut" }}
          className="rounded-2xl border border-[#e2e2ee] overflow-hidden bg-white"
        >
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm min-w-[640px]">
              <thead>
                <tr className="border-b border-[#e2e2ee] bg-[#fafafa]">
                  <th className="text-left px-6 py-4 font-sans text-[10px] font-semibold text-[#8896ab] uppercase tracking-[.16em] w-[240px]">
                    Feature
                  </th>
                  <th className="px-6 py-4 text-center font-sans text-[10px] font-semibold text-[#ff5b35] uppercase tracking-[.16em]">
                    CODEWATCH
                  </th>
                  {COMPETITORS.map((c) => (
                    <th key={c} className="px-6 py-4 text-center font-sans text-[10px] font-semibold text-[#8896ab] uppercase tracking-[.16em]">
                      {c}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {ROWS.map((row, i) => (
                  <motion.tr
                    ...
                    className="border-b border-[#e2e2ee] last:border-0 hover:bg-[#fafafa] transition-colors"
                  >
                    <td className="px-6 py-4 text-sm text-[#0a0f1e]/80">
                      {row.label}
                      {row.sub && (
                        <span className="ml-2 inline-block font-sans text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#ff5b35]/10 text-[#ff5b35] border border-[#ff5b35]/20 tracking-[.08em] align-middle">
                          {row.sub}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-center bg-[#ff5b35]/[0.02]">
